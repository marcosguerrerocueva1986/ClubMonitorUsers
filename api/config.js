...
module.exports = async (req, res) => {
  try {
    const pool = getPool();
    const r = await pool.query(`SELECT onesignal_app_id, icono_deporte, codigo_pais_default FROM sport_control.configuracion_club WHERE id = 1`);
    const row = r.rows[0] || {};
    res.setHeader('Cache-Control', 'public, max-age=300');
    return res.status(200).json({ onesignalAppId: row.onesignal_app_id || null, iconoDeporte: row.icono_deporte || '⚽', codigoPaisDefault: row.codigo_pais_default || '593' });
  } catch (err) {
    console.error('Error en /api/config:', err);
    return res.status(200).json({ onesignalAppId: null, iconoDeporte: '⚽' });
  }
};
