...
async function enviarPushAdmin(pool, titulo, mensaje) {
  try {
    ...
    await fetch('https://onesignal.com/api/v1/notifications', {
      ...
      body: JSON.stringify({
        app_id: c.onesignal_app_id,
        filters: [{ field: 'tag', key: 'es_admin', relation: '=', value: 'true' }],
        target_channel: 'push',
        headings: { en: titulo },
        contents: { en: mensaje },
...
async function obtenerMiPerfil(pool, jugadorId) {
  ...
  return { success: true, data: {
    ...
    eventosHabilitado: row.eventos_habilitado,
    whatsappCheckinNumero: row.whatsapp_checkin_numero,
    representantesHabilitado: row.representantes_habilitado,
    misExentosHabilitado: row.mis_exentos_habilitado,
    miCuentaHabilitado: row.mi_cuenta_habilitado,
    partidosHabilitado: row.partidos_habilitado,
  } };
}
